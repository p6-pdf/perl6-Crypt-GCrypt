...
for (sort keys %dgsts) -> $data {
    for (sort keys %dgsts{$data}) -> $algorithm {
        ...
        $md.write($data);
        my $result = Buf.new: $md.read();
        my $expected = Buf.new: %dgsts{$data}{$algorithm}.comb(/../).map({:16($_)});
...
for (sort keys %hmacs) -> $data {
  for (sort keys %hmacs{$data}) -> $algorithm {
    ...
    die "failed to create HMAC digest object with algorithm $algorithm" unless defined $md;
    $md.write($data);
    my $result = Buf.new: $md.read();
    my $expected = Buf.new: %hmacs{$data}{$algorithm}.comb(/../).map({:16($_)});
    is-deeply $result, $expected;
  }
}
